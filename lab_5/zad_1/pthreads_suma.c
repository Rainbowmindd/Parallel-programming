...
void *suma_w( void *arg_wsk){
  ...
  j=ceil( (float)ROZMIAR/LICZBA_W ); //zaokraglenie w gore do najblizszej calkowitej
  ...
  //Rozwiazanie niepodzielnosci:
  //przedzial dla watku, wtedy nie wyjdzie poza
  int poczatek=(ROZMIAR/LICZBA_W)*moj_id;
  int koniec=(moj_id==LICZBA_W-1)? ROZMIAR: poczatek +(ROZMIAR/LICZBA_W);
  ...
  suma += moja_suma;             //1a bezpieczne sumowanie w sekcji krytycznej PROSTRZE i bardziej uniwersalne
